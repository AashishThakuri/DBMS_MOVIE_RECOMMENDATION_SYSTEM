...
// Zoom Intro Animation - locks body scroll until intro completes
const animateZoomIntro = () => {
  ...
  let virtualScroll = 0;
  const introHeight = window.innerHeight; // Length of intro scroll
  ...
  const handleWheel = (e) => {
    ...
    virtualScroll = Math.max(0, Math.min(virtualScroll, introHeight));

    // Normalize progress
    const progress = virtualScroll / introHeight;

    // Animate overlay
    const scale = 1 + (progress * 2);
    zoomImage.style.transform = `scale(${scale})`;
    zoomOverlay.style.opacity = 1 - progress;
    ...
    if (progress >= 1) {
      ...
      // Prevent default scrolling only if intro is active
      e.preventDefault();
    }
  };

  // Add event listener with passive: false to allow preventing default scroll
...
const init = () => {
  animateZoomIntro();     // Animate the zoom intro overlay
